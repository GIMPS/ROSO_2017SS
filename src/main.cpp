...
int main(void) {
    ...
    try {
       ...
       pthread_create(&threads[0], NULL,guard_thread, (void *)&robo);
       pthread_create(&threads[1], NULL,striker_thread, (void *)&robo);
...
void *striker_thread(void *robo_pointer)
{
    ...
    do
    {
            ...
            striker.GotoXY(pos2.GetX(), pos2.GetY(), 160,true);
            usleep(1000);

    } while (striker.GetPos().DistanceTo(pos2)> 0.1);
    pthread_exit(NULL);
...
void *guard_thread(void *robo_pointer)
{
    ...
    do
    {
            ...
            guard.GotoXY(pos1.GetX(), pos1.GetY(), 160,true);
            usleep(1000);
...
void *goalie_thread(void *robo_pointer)
{
    ...
    while (1) {
                    ...
                    do
                    {
                        ...
                        {
                            goalie.GotoXY(pos2.GetX(), pos1.GetY(), 160,true);
                            usleep(1000);
                        }


                    } while (goalie.GetPos().DistanceTo(pos2)> 0.1);
