    ...
    */
    ...
    try {
        ...
        cout << hex << (int) mac[5] << endl;
         cout << "\t debug2 position: " << robo.GetPos() << endl;
         ...
         */
         cout << "\t debug3 position: " << robo.GetPos() << endl;
        ...
        while (1) {
             ...
             */
            /* kick method that just follows the ball */
            while(robo.GetPos().DistanceTo(ball.GetPos())<1) robo.GotoXY(ball.GetX(),ball.GetY());

            cout << "Moving to " << pos1 << endl << endl;

                        robo.GotoXY(pos1.GetX(), pos1.GetY(), 160, false);
            while (robo.GetPos().DistanceTo(pos1) > 0.10) usleep(sleeptime);

            cout << "Moving to " << pos2 << endl << endl;
                        robo.GotoXY(pos2.GetX(), pos2.GetY(), 160, true);
            while (robo.GetPos().DistanceTo(pos2) > 0.10) usleep(sleeptime);


            cout << "Moving to " << pos3 << endl << endl;
                        robo.GotoXY(pos3.GetX(), pos3.GetY(), 160, false);
            while (robo.GetPos().DistanceTo(pos3) > 0.10) usleep(sleeptime);


            cout << "Moving to " << pos4 << endl << endl;
                        robo.GotoXY(pos4.GetX(), pos4.GetY(), 160, true);
            while (robo.GetPos().DistanceTo(pos4) > 0.10) usleep(sleeptime);

            cout << "\t ball position: " << ball.GetPos() << endl;
